...
const Create = () => {
  ...
  const [inputValidationMiddleName, setInputValidationMiddleName] =
    useState(false);
  const [inputValidationFromDate, setInputValidationFromDate] = useState(false);
  ...
  useEffect(() => {
    ...
    setInputValidationFromDate(
      moment(getFormattedTime(certificateData.from), "MM.DD.YYYY").isValid()
    );
  ...
  const handleViewCertificate = async () => {
    ...
    if (
      ...
      try {
        ...
        setCertificateStatus(createStatusInstance("error", t("status.error")));
      }
    } else {
      setCertificateStatus(createStatusInstance("error", t("status.error")));
    }
  ...
  const handleCreateCertificate = async (e) => {
    ...
    try {
      ...
      if (response.data.id) {
        ...
        setCertificateStatus(
          createStatusInstance("success", t("status.success"), response.data.id)
        );
        setCreateStep("save");
      }
    } catch (error) {
  ...
  return (
    <div admincontent="content" className="create-certificate">
      ...
      <section className="create-certificate__content">
        ...
        >
          <div className="create-certificate__form-fields">
            ...
            >
              ...
              <input
                ...
                required
              />
              {!inputValidationName && certificateStatus.status === "error" && (
            ...
            <div
              ...
              inputwrapperitem="true"
            >
              <label htmlFor="surname">{t("create.surname")}</label>
              <input
                type="text"
                id="surname"
                autoComplete="off"
            ...
            <div
              ...
              inputwrapperitem="true"
            >
              <label htmlFor="certificate-type">{t("create.certificate-type")}</label>
              <select
                defaultValue={certificateData.certificateType || "select"}
                onChange={(e) =>
                  setCertificateData({
                    ...certificateData,
              ...
              >
                ...
                <option value="driver-certificate">{t("create.driver-certificate")}</option>
                <option value="driver-manager-certificate">{t("create.driver-manager-certificate")}</option>
                <option value="driver-adr-certificate">{t("create.driver-adr-certificate")}</option>
                <option value="driver-adr-tank-certificate">{t("create.driver-adr-tank-certificate")}</option>
              </select>
              {certificateData.certificateType === "select" &&
                certificateStatus.status === "error" && (
                  <p className="message message--error">
            ...
            >
              ...
              <DatePicker
                ...
                onChange={(date) =>
                  setCertificateData({ ...certificateData, from: date })
                }
                required
              />
            ...
            >
              ...
              <DatePicker
                ...
                onChange={(date) =>
                  setCertificateData({ ...certificateData, to: date })
                }
                required
              />
          ...
          <div className="create-certificate__form-actions">
            <Button
              ...
              loading={certificateStatus.loading && createStep === "backlog"}
        ...
        {certificateStatus.status === "success" && certificateStatus.data ? (
          <div className="create-certificate__view">
            ...
            {certificateData.certificateType === "driver-certificate" ||
            certificateData.certificateType === "driver-manager-certificate" ? (
              <DriverCertificate
                ...
                pdf_class={
                  createStep === "view" ? "pdf_mainContainer" : undefined
                }
                ref={printFrame}
              />
            ) : (
