...
const CertificatesTable = () => {
            ...
            try {
                ...
                if (pathname.includes(endpoint)) {
                    const response = await axios.get(
                        `/${endpoint.replace(/-/g, '')}/all?limit=${certificateLimit}&page=${currentPageNumber}`
                    )
                    if (response.status === 200) {
                        ...
                        throw new Error(
                            'No certificates found or something went wrong'
                        )
                    }
                }
            } catch (error) {
    ...
    useEffect(() => {
        ...
        async function loadSearchResults() {
            try {
                const response = await axios.get(
                    `/search/search-by-all?${identifySearchValueType(search)}&type=${type}&limit=${certificateLimit}&page=${currentPageNumber}`
                )
    ...
    const handleDeleteCertificate = () => {
        try {
            axios
                .patch(
                    `/${pathname.split('/')[3].replace('manage-', '').replace(/-/g, '')}/delete/${deletedata._id}`
                )
                .then(response => {
                    if (response.status === 200) {
                        modal.current.close()
                    ...
                    setCertificateDataResult(
                        createStatusInstance('success', t('status.success'))
                    )
                })
        } catch (err) {
            console.log(err)
    ...
    return (
        <div className="certificates">
            <div className="certificates__filters">
                <form className="certificates__filters-form">
                    <div className="certificates__filters-search">
                        <label htmlFor="search-sertificate">
                            Search certificate by ID or Name
                        </label>
                        <input
                            type="text"
                            id="search-sertificate"
                            value={search}
                ...
                <div className="certificates__filters-pagination">
                    <select
                        disabled={certificateDataResult.loading}
                        value={certificateLimit}
                        className="certificates__filters-pagination-limit"
                        onChange={e => setCertificateLimit(+e.target.value)}
                    >
                        {new Array(10).fill('').map((item, index) => (
                            <option key={uuidv4()} value={(index + 1) * 10}>
                                {(index + 1) * 10}
                            </option>
                        ))}
                    </select>
                    <button
                        disabled={certificateDataResult.loading}
                        className="certificates__filters-pagination-btn"
                        onClick={handlePreviousPageFetching}
                    >
                        <BsChevronLeft />
                    </button>
                    <p className="certificates__filters-pagination-number">
                        {currentPageNumber}
                    </p>
                    <button
                        disabled={certificateDataResult.loading}
                        className="certificates__filters-pagination-btn"
                        onClick={handleNextPageFetching}
                    >
                        <BsChevronRight />
                    </button>
                </div>
                ...
                <table className="certificates__table">
                    ...
                    <tbody>
                        {certificateDataResult.data &&
                            certificateDataResult.data.allCertificates.map(
                                certificate => {
                                    return (
                                        <tr
                                            className="certificates__table-row"
                                            key={certificate._id}
                                        >
                                            ...
                                            <Modal
                                                ref={modal}
                                                title={`Delete this certificate? ${deletedata?.id}`}
            ...
            <div style={{ visibility: 'hidden', width: 0, height: 0 }}>
                {downloadData && (
                    <>
                        {downloadData?.id.startsWith('M') ||
                        downloadData?.id.startsWith('D') ? (
                            <DriverCertificate
                                id={downloadData.id}
