...
const DriverCardCertificates = require('../models/DriverCardCertificate')
...
search.get('/', async (req, res) => {
    ...
    let certificate
    try {
        if (id.toLowerCase().includes('dc')) {
            certificate = await DriverCardCertificates.findOne({ id })
        } else if (id.toLowerCase().includes('d')) {
