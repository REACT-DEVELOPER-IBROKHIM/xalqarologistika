...
search.get('/search-by-all', verifyAdmin, async (req, res) => {
    ...
    try {
        ...
        switch (type) {
            case 'certificate':
                certificates = await DriverCertificates.find(query)
                    .limit(limit)
                    .skip(limit * (page - 1))
            ...
            case 'adr-certificate':
                certificates = await AdrCertificates.find(query)
                    .limit(limit)
                    .skip(limit * (page - 1))
            ...
            case 'adr-tank-certificate':
                certificates = await AdrTankCertificates.find(query)
                    .limit(limit)
                    .skip(limit * (page - 1))
            ...
            case 'manager-certificate':
                certificates = await ManagerCertificates.find(query)
                    .limit(limit)
                    .skip(limit * (page - 1))
