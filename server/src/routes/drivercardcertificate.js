...
const Certificate = require('../models/DriverCardCertificate')
...
certificate.post('/', verifyAdmin, async (req, res) => {
    try {
        ...
        const newCertificate = await Certificate.create({
            id: generateId(allCertificates + 1, 7, 'DC'),
            name: req.body.name,
            surname: req.body.surname,
            middlename: req.body.middlename,
...
certificate.get('/document-count', verifyAdmin, async (req, res) => {
    try {
        const totalCertificates = await Certificate.countDocuments({})
        res.status(200).json({
            data: generateId(totalCertificates + 1, 7, 'DC'),
            error: null,
        ...
        const removedCert = await Certificate.updateOne(
            ...
            },
            {
                $set: {
                    ...
                    driverLicenceNumber: 'Mavjud emas',
