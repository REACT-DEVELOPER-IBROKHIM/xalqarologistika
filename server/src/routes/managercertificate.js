...
mCertificate.post("/", verifyAdmin, async (req, res) => {
  ...
  const existCertificate = await ManagerCertificate.findOne({name: req.body.name, surname: req.body.surname, middlename: req.body.middlename});
  if(existCertificate){
...
mCertificate.get("/all", verifyAdmin,  async (req, res) => {
    ...
    try {
      ...
      const allCertificates = await ManagerCertificate.find().limit(limit).skip(limit * (page - 1));
