...
certificate.post("/", verifyAdmin, async (req, res) => {
  ...
  const existCertificate = await Certificate.findOne({name: req.body.name, surname: req.body.surname, middlename: req.body.middlename});
  if(existCertificate){
...
certificate.get("/all", verifyAdmin, async (req, res) => {
    ...
    try {
      ...
      const allCertificates = await Certificate.find().limit(limit).skip(limit * (page - 1));
